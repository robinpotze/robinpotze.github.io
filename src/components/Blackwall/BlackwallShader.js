...
void main() {
    ...
    float wave = sin(uv.x * 5.0 + u_time * 0.5) * 0.1;
    uv.x += wave;

    float scanline = sin(uv.y * u_resolution.y * 0.5 + u_time * 50.0) * 0.05;
    uv.x += scanline;

    float band = floor(uv.y * 20.0);
    float enableBand = step(0.8, fract(u_time * 0.1 + band * 0.9));
    uv.x += sin(u_time * 10.0 + band) * 0.02 * enableBand;

    vec2 redUV = uv + vec2(0.005, 0.0);
    vec2 blueUV = uv - vec2(0.005, 0.0);

    vec2 distortedUV = uv + noise(uv * 10.0 + u_time) * 0.05;
    float layeredNoise = noise(distortedUV * 5.0) * 0.5 + noise(distortedUV * 20.0) * 0.5;

    float flicker = step(1.0, fract(sin(u_time * 10.0) * 43758.5453123));

    float mask = circularMask(uv, u_mouse, 0.15);
    ...
    if (uv.y < floorLine) {
        ...
        vec2 reflectedUV = uv;
        reflectedUV.y = floorLine - (uv.y - floorLine); // Mirror the UV coordinates below the floor line
        ...
        reflectionColor.g += uv.x * flicker;
        reflectionColor.b += blueUV.x * flicker;

        // Reflection falloff based on distance from the bottom
        float reflectionFalloff = smoothstep(0.0, floorLine, uv.y); // Gradually reduce reflection closer to the bottom
