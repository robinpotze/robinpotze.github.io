import{r as u,j as J}from"./router_motion-2JVGeciU.js";import{S as N,O as K,W as Q,g as Z,P as $,V as E,a as X,M as j}from"./three-QGPY2NKn.js";import"./TopNav-2JbvURzl.js";import"./react-Bzgz95E1.js";import"./index-CR9BiYi5.js";function ee({initialScale:x=1,enabled:p=!0,minScale:F=.25,maxScale:U=1,adaptIntervalMs:V=1e3,fpsSampleSize:y=30,targetFps:R=30,adaptThreshold:P=.85,adaptStep:b=.85}={}){const l=u.useRef(x),d=u.useRef([]),s=u.useRef(performance.now()),c=u.useRef(p);function C(){return l.current}function h(i){l.current=Math.max(F,Math.min(U,i))}function e(){c.current=!1}function g(){c.current=!0}function w(i){if(!c.current)return null;const a=d.current;a.push(i),a.length>y&&a.shift();const m=performance.now();if(m-s.current<V)return null;if(a.length>=Math.max(4,Math.floor(y/3))){const _=1e3/(a.reduce((T,S)=>T+S,0)/a.length);if(_<P*R)return h(l.current*b),d.current.length=0,s.current=m,l.current;if(_>R*1.05)return h(l.current/b),d.current.length=0,s.current=m,l.current}return null}return u.useEffect(()=>{c.current=p},[p]),{getScale:C,setScale:h,sampleFrame:w,start:g,stop:e}}const ne=`precision mediump float;
...
void main(){
 ...
 `;function se({renderScale:x=1,resizeDebounceMs:p=100,adaptivityEnabled:F=!0,minRenderScale:U=.25,maxRenderScale:V=1,adaptIntervalMs:y=1e3,fpsSampleSize:R=30,adaptThreshold:P=.85,adaptStep:b=.85}={}){const l=u.useRef(),d=ee({initialScale:x,enabled:F,minScale:U,maxScale:V,adaptIntervalMs:y,fpsSampleSize:R,targetFps:30,adaptThreshold:P,adaptStep:b}),s=u.useRef(d.getScale());return u.useEffect(()=>{const c=l.current;if(!c)return;const C=new N,h=new K(-1,1,1,-1,0,1),e=new Q({antialias:!0,alpha:!1});e.setClearColor(0,1),e.setPixelRatio(window.devicePixelRatio||1);const g=Math.max(1,Math.floor(window.innerWidth*x)),w=Math.max(1,Math.floor(window.innerHeight*x));e.setSize(g,w,!1),e.domElement.classList.add("blackwall-canvas"),c.appendChild(e.domElement);const i=e.getPixelRatio()||window.devicePixelRatio||1,a=new Z(Math.max(1,Math.floor(g*i)),Math.max(1,Math.floor(w*i))),m=new $(2,2),M={u_time:{value:0},u_resolution:{value:new E(Math.max(1,Math.floor(g*i)),Math.max(1,Math.floor(w*i)))},u_mouse:{value:new E(.5,.5)}},_=new X({uniforms:M,fragmentShader:oe,transparent:!0}),T=new j(m,_),S=new N;S.add(T);const k={u_time:{value:0},u_resolution:{value:new E(Math.max(1,Math.floor(g*i)),Math.max(1,Math.floor(w*i)))},u_mouse:{value:new E(.5,.5)},u_flame:{value:a.texture}},H=new X({uniforms:k,fragmentShader:ne,transparent:!0}),O=new j(m,H);C.add(O);let q=performance.now(),W=0;const Y=30;let L=s.current;function A(r){const o=Math.max(U,Math.min(V,r));if(Math.abs(o-L)<.001)return!1;s.current=o,L=o,d.setScale(o);const t=window.innerWidth,n=window.innerHeight;e.domElement.classList.contains("blackwall-canvas")||e.domElement.classList.add("blackwall-canvas");const f=Math.max(1,Math.floor(t*L)),D=Math.max(1,Math.floor(n*L));return e.setPixelRatio(window.devicePixelRatio||1),e.setSize(f,D,!1),v&&(clearTimeout(v),v=null),z(f,D),!0}function B(){requestAnimationFrame(B);const r=performance.now(),o=r-W;if(o<1e3/Y)return;W=r;const t=(r-q)*.001,n=d.sampleFrame(o);n!=null&&A(n),M.u_time.value=t,k.u_time.value=t,e.setRenderTarget(a),e.render(S,h),e.setRenderTarget(null),e.render(C,h)}A(s.current),B();function G(r){const o=window.innerWidth/window.innerHeight,t=r.clientX/window.innerWidth,n=1-r.clientY/window.innerHeight,f=(t-.5)*o+.5;M.u_mouse.value.set(f,n),k.u_mouse.value.set(f,n)}let v=null;function z(r,o){const t=e.getPixelRatio()||window.devicePixelRatio||1,n=Math.max(1,Math.floor(r*t)),f=Math.max(1,Math.floor(o*t));a?.setSize?.(n,f),M.u_resolution.value.set(n,f),k.u_resolution.value.set(n,f)}function I(){const r=window.innerWidth,o=window.innerHeight;e.domElement.classList.contains("blackwall-canvas")||e.domElement.classList.add("blackwall-canvas");const t=Math.max(1,Math.floor(r*s.current)),n=Math.max(1,Math.floor(o*s.current));e.setPixelRatio(window.devicePixelRatio||1),e.setSize(t,n,!1),p<=0?z(t,n):(v&&clearTimeout(v),v=setTimeout(()=>{z(t,n),v=null},p))}return window.addEventListener("mousemove",G),window.addEventListener("resize",I),()=>{window.removeEventListener("mousemove",G),window.removeEventListener("resize",I),c&&e.domElement&&c.removeChild(e.domElement),m.dispose(),_.dispose(),a.dispose(),H.dispose(),e.dispose()}},[]),J.jsx("div",{ref:l,className:"fixed inset-0 -z-10 pointer-events-none"})}export{se as default};
